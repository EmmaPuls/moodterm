...
class OSCMiddleware: TerminalMiddleware {
    var cwdReported = PassthroughSubject<String, Never>()
    ...
    override func feedFromSession(data: Data) {
        ...
        while let prefixRange = data.range(
            of: OSCPrefix, options: [], in: startIndex..<data.endIndex),
            let suffixRange = data.range(
                of: OSCSuffix, options: [], in: prefixRange.upperBound..<data.endIndex)
        {
            ...
            let components = oscString.split(
